class Capybara::Puppeteer::Node <  Capybara::Driver::Node
  # ref:
  ...
  def find_xpath(query, **options)
    @page.wait_for_function(
      '(query, scopeElement) => document.evaluate(query, scopeElement, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength > 0',
      args: [query, @element],
    )
    @element.Sx(query).map do |el|
      ::Capybara::Puppeteer::Node.new(@driver, @page, el)
    end
  end
